...
describe('ContributionsSection', () => {
  const blogs = [
    { id: 1, strTitle: 'Blog One', strSummary: 'Summary one', strThemeColor: '#10b981', strMediaUrl: null },
    { id: 2, strTitle: 'Blog Two', strSummary: 'Summary two', strThemeColor: '#3b82f6', strMediaUrl: '/test.jpg' },
    { id: 3, strTitle: 'Blog Three', strSummary: 'Summary three', strThemeColor: '#10b981', strMediaUrl: null },
    { id: 4, strTitle: 'Blog Four', strSummary: 'Should not show', strThemeColor: '#10b981', strMediaUrl: null },
  ];
  ...
  it('shows article count in footer', () => {
    wrap(<ContributionsSection arrBlogsState={blogs} />);
    expect(screen.getByText('4 featured articles')).toBeDefined();
  });

  it('uses fallback image when strMediaUrl is null', () => {
    const { container } = wrap(<ContributionsSection arrBlogsState={blogs} />);
    const images = container.querySelectorAll('.ath-blog-img');
    expect(images[0].src).toContain('/blog-images/trust-digital-discourse.svg');
  });

  it('uses provided strMediaUrl when available', () => {
    const { container } = wrap(<ContributionsSection arrBlogsState={blogs} />);
    const images = container.querySelectorAll('.ath-blog-img');
    expect(images[1].src).toContain('/test.jpg');
  });

  it('assigns correct category based on theme color', () => {
    wrap(<ContributionsSection arrBlogsState={blogs} />);
